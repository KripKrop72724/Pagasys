...
def _day_bounds(day: date) -> tuple[datetime, datetime]:
    """Return window covering 18:00 on D-1 through 06:00 on D+1."""
    start = datetime.combine(day, time.min, tzinfo=dt_timezone.utc) - timedelta(hours=6)
...
def pair_events_task(company_id: int, employee_id: int, day: str | date) -> None:
    ...
    paired_ids = set(
        ev_id
        for ev_id_pair in AttPair.objects.filter(employee_id=employee_id).values_list(
            "in_event_id", "out_event_id"
        )
        for ev_id in ev_id_pair
        if ev_id
    )

    events = [
        ev
        for ev in AttEvent.objects
        .filter(company_id=company_id, employee_id=employee_id, ts__gte=start, ts__lt=end)
        .order_by("ts")
        if ev.id not in paired_ids
    ]
